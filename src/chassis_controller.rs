...
impl ChassisController {
    pub fn stick_to_motor_power(stick_position: StickPosition) {
        ...
        defmt::debug!("normalized stick positions: x={:?}, y={:?}", normalized_x, normalized_y);
        defmt::debug!("motor powers: left={:?}, right={:?}", left, right);
    }
}
